package org.firstinspires.ftc.teamcode;

import com.acmerobotics.dashboard.config.Config;

@Config
public class RobotVars {
    public static double SDESCHIS = 0.73;
    public static double SINCHIS = 0.85;

    public static int EMIN = 0;
    public static int EMAX = 1000;

    public static int RTOP_POS = 1000;
    public static int RMIU_POS = 728;
    public static int RMID_POS = 450;
    public static int RBOT_POS = 100;

    public static double S1PC = 0.72;
    public static double S2PC = 0.28;
    public static double S3PC = 0.486;

    public static double S1PO = 1;
    public static double S2PO = 0.04;
    public static double S3PO = 0.54;

    public static double SAG = 0.54;
    public static double SBG = 0.54;
    public static double SHG = 0.54;
    public static double SAP = 0.54;
    public static double SBP = 0.54;
    public static double SHP = 0.54;
    public static double SCO = 0.54;
    public static double SCC = 0.54;

    public static boolean USE_PHOTON = true;

    public static boolean USE_TELE = true;
    public static double pcoef = 1.0;
    public static boolean coneReady = false;
}
